function [Vanout, Vanout_new, tout_new, Iout_new, tout, Iout, f, P1,...
    P1_V, THD_percentage, simOut] = partfourierexperiment_1 ...
    (stepsize, V, frequency, R, L, fs, Vdc, stoptime, samplefrequency,...
    u_sampletime)
...
% further processing. partfourierexperiment_1 is the name of
...
% frequency of the carrier signal it can be sawtooth wave (Bipolar SVPWM) or triangular
% wave (unipolar SVPWM). Vdc is the DC-link volatge. stoptime is the simulation stoptime of
...
simOut = sim("SVPWM_self.slx");
% simOut is the Simulation output of the Simulink file  SVPWM_self.slx
Iout = simOut.Iout(:,1);
Vanout = simOut.Vanout;
tout = simOut.tout;
...
% Vanout is a matrix smimilat to Iout, which contains phase a output voltage
...
k3 = uint64((5*t_lr)/stepsize);
k4 = uint64((1/frequency)/stepsize);
...
% state,5*t_lr/stepsize is the number of sample point before transient state
...
% line 56 to 63 use the same methods as line 83 to 86, since the FFT analysis only
...
% calcualte the number of intervals is, P1 is a matrix whcich save the value
% of FFT spectrum as structure of time, when how many intervals betwenn n th
...
% total number of interval of the FFT spectrum in frequency achse, k6 divide k8 indicates
...
% P3_square make the square of all the data in P3, k10 is the  sum of p3_square. 
...
% is equal to k10-k11 and is the spectrum of the first harmonic
